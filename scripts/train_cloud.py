"""CLI: train the Cloud specialist on CIC-IDS2018 (preprocess → CNN+LSTM+Attention)."""
...
def parse_args() -> argparse.Namespace:
    """Parse command-line arguments for Cloud specialist training."""
    p = argparse.ArgumentParser(description="Train Cloud traffic specialist (configs/cloud_model.yaml).")
    p.add_argument(
        "--nrows",
        type=int,
        default=None,
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="Random seed (default: datasets.cloud.random_state).",
    )
    return p.parse_args()


def main() -> None:
    """Load Cloud data, split, fit preprocessor, train specialist, save model."""
    args = parse_args()
    ds_cfg = load_config("datasets")["cloud"]
    ...
    df = load_dataset_raw("cloud", nrows=args.nrows)
    ...
    logger.info("Cloud specialist training finished.")
